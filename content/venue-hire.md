+++
title = "Nanpantan Scout Hut - Venue Hire | Parties & Events in Loughborough"
description = "Hire the Nanpantan Scout Hut for your next event. Modern spacious hall, meeting room, kitchen facilities, and parking available. Ideal for parties, classes, and community groups."
keywords = ["hall hire", "Loughborough", "kids parties", "community center", "Charnwood", "event venue"]
authors = ["Carl Travis-Roberts"]
+++

# Nanpantan Scout Hut Venue Hire

## The ideal Loughborough venue for your party or event

**Welcome to the 1st Nanpantan Scout Hut.**

Perfect for children's parties, yoga classes, committee meetings, and community events, our modern building offers flexible spaces with excellent facilities on the edge of Loughborough.

* **Brand New Building:** Modern, clean facilities built to the highest standards.
* **Parking Available:** Convenient on-site parking for easy access.
* **Kitchen Facilities:** Fully equipped kitchen for catering and refreshments.
* **Flexible Spaces:** Main hall and meeting room options to suit your needs.

![Nanpantan Scout Hut](/img/venue/front.jpg)

---

## Perfect For...

### Children's or Family Parties
* Spacious main hall with room to run around.
* Kitchen access included for party food.
* Indoor and outdoor areas available.
* Available until 10pm (11pm Saturdays).
* **Bouncy castles welcome!**

### Regular Classes
* Ideal for yoga, fitness, and dance classes.
* Flexible booking arrangements.
* Clean, warm, and modern environment.
* Convenient monthly payment options.

### Meetings & Groups
* Dedicated meeting room separate from the hall.
* Available during weekdays.
* Professional setting ideal for committees.

---

## Our Facilities

<table style="width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 1em; font-family: sans-serif;">
  <thead>
    <tr style="background-color: #0095c6; color: #ffffff; text-align: left;">
      <th style="border: 1px solid #dddddd; padding: 12px 15px;">Space</th>
      <th style="border: 1px solid #dddddd; padding: 12px 15px;">Capacity & Details</th>
      <th style="border: 1px solid #dddddd; padding: 12px 15px;">Learn More</th>
    </tr>
  </thead>
  <tbody>
    <tr style="border-bottom: 1px solid #dddddd;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><strong>Main Hall</strong></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Capacity: 242 people (Standing)</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><a href="#pricing">View Pricing</a></td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd; background-color: #f9f9f9;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><strong>Meeting Room</strong></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Capacity: 10 people</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><a href="#pricing">View Pricing</a></td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><strong>Kitchen</strong></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Fully equipped (Oven, hob, fridge, kettle)</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><a href="#pricing">View Pricing</a></td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd; background-color: #f9f9f9;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><strong>Outdoor Area</strong></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Patio and grassy grounds</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><a href="#pricing">View Pricing</a></td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><strong>Parking</strong></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">~10 On-site spaces</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><a href="#pricing">View Pricing</a></td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd; background-color: #f9f9f9;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><strong>Amenities</strong></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">WiFi, 3 unisex toilets, 1 accessible toilet</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><a href="#pricing">View Pricing</a></td>
    </tr>
  </tbody>
</table>

{{< hut_gallery images="/img/venue/front.jpg::Venue Front,/img/venue/main-hall.jpg::Spacious Main Hall,/img/venue/meeting-room.jpg::Meeting Room,/img/venue/kitchen.jpg::Fully Equipped Kitchen,/img/venue/outside.jpg::Outside Area" >}}

---
## Pricing

Find our standard pricing below, we do have custom rates for registered charities and Scout / Guide groups - please get in touch to discuss these. 

### Standard Community Rates

<table style="width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 1em; font-family: sans-serif;">
  <thead>
    <tr style="background-color: #0095c6; color: #ffffff; text-align: left;">
      <th style="border: 1px solid #dddddd; padding: 12px 15px;">Rate Type</th>
      <th style="border: 1px solid #dddddd; padding: 12px 15px;">Hall (£)</th>
      <th style="border: 1px solid #dddddd; padding: 12px 15px;">Meeting Room (£)</th>
    </tr>
  </thead>
  <tbody>
    <tr style="border-bottom: 1px solid #dddddd;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><strong>Weekday</strong></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"></td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd; background-color: #f9f9f9;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">One off Booking (Hourly)</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">20</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">15</td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Recurring Booking (Hourly)<sup>[1]</sup></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">15</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">10</td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd; background-color: #f9f9f9;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Full Day</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">140</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">125</td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"><strong>Weekend</strong></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;"></td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd; background-color: #f9f9f9;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">One off Booking (Hourly)</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">25</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">15</td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Recurring Booking (Hourly)<sup>[1]</sup></td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">25</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">10</td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd; background-color: #f9f9f9;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Full Day (8am-5pm)</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">175</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">125</td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Evening (5.30pm-10pm)</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">135</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">N/A</td>
    </tr>
    <tr style="border-bottom: 1px solid #dddddd; background-color: #f9f9f9;">
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">Evening (5.30pm-11pm)</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">165</td>
      <td style="border: 1px solid #dddddd; padding: 12px 15px;">N/A</td>
    </tr>
  </tbody>
</table>

[1] Weekly booking more than 4 weeks, monthly booking more than 4 months.


## Ready to Book?

<style>
  .scout-booking-container {
    display: flex;
    flex-wrap: wrap;
    gap: 30px;
    margin: 40px 0;
    font-family: sans-serif;
  }
  .scout-booking-box {
    flex: 1;
    min-width: 280px; /* Stacks on mobile */
    background: #f9f9f9;
    padding: 25px;
    border-radius: 8px;
    border: 1px solid #eee;
    text-align: center;
  }
  .scout-btn {
    display: inline-block;
    margin-top: 15px;
    padding: 12px 25px;
    font-size: 16px;
    font-weight: bold;
    text-decoration: none;
    border-radius: 5px;
    transition: all 0.2s ease;
  }
  
  /* Primary Button (Parties) - Scout Purple */
  .scout-btn-primary {
    background-color: #7413dc; /* Scout Purple */
    color: #ffffff !important; /* Force white text */
    border: 2px solid #7413dc;
  }
  .scout-btn-primary:hover {
    background-color: #580ba8;
    border-color: #580ba8;
    transform: translateY(-2px);
    text-decoration: none;
  }

  /* Secondary Button (Regular) - Outline Style */
  .scout-btn-secondary {
    background-color: transparent;
    color: #0095c6 !important; /* Scout Teal */
    border: 2px solid #0095c6;
  }
  .scout-btn-secondary:hover {
    background-color: #0095c6;
    color: #ffffff !important;
    text-decoration: none;
  }
  
  .scout-booking-box h3 {
    margin-top: 0;
    color: #333;
  }
</style>

<div class="scout-booking-container">
  
  <div class="scout-booking-box">
    <h3>🎉 One-off Events</h3>
    <p>Looking for a venue for a birthday party, family gathering, or single event?</p>
    <a href="LINK_TO_BOOKING_SYSTEM" class="scout-btn scout-btn-primary">Check Availability & Book</a>
  </div>

  <div class="scout-booking-box">
    <h3>📅 Regular Classes & Groups</h3>
    <p>Want to host a weekly yoga class, club, or committee meeting?</p>
    <a href="LINK_TO_ENQUIRY_FORM" class="scout-btn scout-btn-secondary">Enquire About Hiring</a>
  </div>

</div>
<p style="text-align: center; margin-top: 20px;">
  <strong>Questions?</strong> Email us at <a href="mailto:bookings@nanpantanscouts.org.uk">bookings@nanpantanscouts.org.uk</a>
</p>

---

## Calender

EMBED

## Frequently Asked Questions

**What are the opening hours?**
The hall can be hired until 10pm Sunday–Friday, and until 11pm on Saturdays. Please ensure your booking period includes adequate time for both setting up and clearing away.

**How do I pay for my booking?**
Payment is required by bank transfer to secure your booking.
* **One-off bookings:** Full payment and deposit are due at the time of booking.
* **Recurring bookings:** One month's hire is paid upfront, followed by monthly invoices payable within 28 days.

**What is your cancellation policy?**
Cancellations must be notified **4 weeks in advance** of your booking for a full refund of the booking fee.

**Can I bring a bouncy castle?**
Yes! Bouncy castles and inflatables are allowed both inside and outside (maximum size 5m x 5m, height 4m). Outside inflatables must be positioned to minimise intrusion to neighbours and deflated by 9pm. You must ensure proper supervision and that your supplier provides suitable insurance.

**Can I serve alcohol at my event?**
You cannot *sell* alcohol without a Temporary Event Licence from Charnwood Borough Council. Alcohol consumption by persons under 18 is strictly not permitted.

**Can I play music?**
Yes, but windows and doors must be kept shut if amplified music is played inside.
* **Inside:** Amplified music is permitted.
* **Outside:** No amplified music at any time. Low-level background music is acceptable between 8am and 6pm.

**Is smoking allowed?**
No. Smoking and vaping (including e-cigarettes) are not permitted anywhere inside the building.

**Can I use decorations?**
* **Walls:** You must not use blu-tack, Sellotape, drawing pins, or similar items on the building fabric.
* **Hanging:** Bunting and balloons can be attached with string to specific hooks provided in the main hall.
* **Glitter/Confetti:** Permitted inside, but must be fully cleaned up at the end of your hire to avoid deduction from your deposit.

**Are pets allowed?**
Guide dogs and assistance dogs are welcome (though assistance dogs cannot enter the kitchen area). Other animals are generally not permitted unless agreed upon in advance for a special event.

**Can I have a BBQ?**
Disposable barbecues are not permitted. Standard barbecues may be set up on the patio area only.

**What items are not permitted?**
* Fireworks
* Smoke, foam, or bubble-making machines
* Disposable barbecues

**What are my responsibilities at the end of hire?**
You must leave the premises clean and tidy. This includes:
1.  Closing all doors and windows.
2.  Switching off lights and appliances.
3.  Returning keys to the key safe.
4.  Taking all rubbish away with you (recycling can go in the external bin *only* if not full).
Your deposit will be returned within 5 working days once these conditions are met.

**Do I need insurance?**
* **Commercial/Regular Hirers:** Yes, you are responsible for arranging insurance against third-party claims.
* **Private Parties:** Please check your own cover. The Scout Group is insured against claims arising from its own negligence only, not yours.